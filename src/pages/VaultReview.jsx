...
const QuickNavArrow = ({ item, type, onNavClick }) => {
  ...
  return (
    <div 
      onClick={(e) => { e.stopPropagation(); onNavClick(item._id, type); }} 
    ...
    return (
      <div className="relative w-full h-full bg-black">
        <div className="fixed top-0 left-0 w-full z-50 overflow-hidden shadow-2xl pointer-events-none" style={{ height: heroHeight + 'px' }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6" style={{ transform: 'scale(' + textScale + ')' }}>
            ...
            <div className="mt-8 absolute -bottom-12 transition-opacity duration-300" style={{ opacity: 1 - (progress * 3) }}>
  ...
  return (
    <div onMouseMove={handleMouseMove} className={`absolute inset-0 w-full h-screen overflow-hidden bg-black text-white transition-transform duration-700 will-change-transform ${transform}`} style={{ transitionTimingFunction: 'cubic-bezier(0.33, 1, 0.68, 1)' }}>
       <div onClick={() => (isReading && setIsReading(false))} onWheel={(e) => !isReading && e.deltaY > 20 && setIsReading(true)} className={`relative transition-all duration-700 ease-[cubic-bezier(0.33,1,0.68,1)] overflow-hidden shrink-0 z-10 shadow-[0_10px_60px_rgba(0,0,0,1)] ${desktopImageClasses}`}>
        ...
        <div className={`absolute bottom-12 left-0 right-0 flex justify-center transition-all duration-700 ${isVisualReading ? 'opacity-0 translate-y-10 pointer-events-none' : 'opacity-100 translate-y-0'} z-30`}>
...
const VaultReview = () => {
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      {outgoingId && <ReviewSlide review={vaultItems.find(r => r._id === outgoingId)} isReading={isReading} isExiting={true} active={false} transform={getPageTransform(outgoingAnimState)} onNavClick={()=>{}} isMobile={isMobile} vaultItems={vaultItems} />}

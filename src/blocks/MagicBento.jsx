...
const MagicBento = memo(({ items }) => {
  ...
  // Items are already sorted by Home.jsx, so we just slice
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-8 pt-4 pb-20" style={{ perspective: '1200px' }}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
        {displayItems.map((item, index) => {
          ...
          return (
            <div key={item.id || index} className={`relative ${isHero ? 'md:col-span-2 h-[400px] md:h-[600px]' : 'h-[300px] md:h-[400px]'}`}>
              <Link 
                ...
                style={{ transformStyle: 'preserve-3d', transform: isHovered ? `rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)` : 'rotateX(0deg) rotateY(0deg)' }}
              >
                {/* IMAGE CONTAINER */}
                <div className={`absolute inset-0 z-0 rounded-[2rem] md:rounded-[3.5rem] overflow-hidden bg-black transition-all duration-500 border ${isHovered ? 'border-[#5227ff]/60 shadow-[0_0_40px_rgba(82,39,255,0.3)]' : 'border-white/10'}`} style={{ transform: 'translateZ(0px)' }}>
                  <img src={optimizedUrl} className={`h-full w-full object-cover transition-all duration-700 ease-in-out ${isHovered ? 'opacity-100 scale-105 saturate-110' : 'opacity-80 scale-100 saturate-[0.85]'}`} alt={item.title} loading="lazy" />
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 z-30 pointer-events-none" style={{ transform: 'translateZ(0px)' }}>
                  <div className={`px-5 py-2 rounded-b-xl border-x border-b backdrop-blur-xl font-mono text-[9px] md:text-[10px] uppercase tracking-[0.2em] transition-all duration-500 -mt-[1px] ${isHovered ? 'bg-[#5227ff]/20 border-[#5227ff]/60 text-white shadow-[0_0_20px_rgba(82,39,255,0.4)]' : 'bg-black/60 border-white/10 text-white/70'}`}>
                    :: {item.type}
                  </div>
                </div>
                ...
                {/* QUOTE (Hero Only) */}
                {!isMobile && isHero && (
                  <div className={`absolute inset-0 flex items-center justify-center pointer-events-none text-center transition-all duration-500 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transform: `translateZ(80px)`, padding: '0 5%' }}>
                    <p className="font-editorial italic font-bold text-white drop-shadow-[0_4px_20px_rgba(0,0,0,1)] text-lg md:text-2xl whitespace-pre-wrap w-full max-w-2xl leading-tight">"{item.quote}"</p>
                  </div>
                )}
